...
# CA로 초고속 키이미지 생성
# 5nCA key image generator

def FNCAKIG(size, param):
        """
        FNCAKIG
        ...
        * rule에 대한 이슈
        ...
        for z in range(3):
                ...
                for j in range(d.shape[1]):
                        ...
                        h=np.bitwise_xor(np.right_shift(x,2),np.left_shift(x,2))
                        r=np.bitwise_and(x,rule[z])
                        kh=np.bitwise_xor(k,h)
                        d[i,j,z]=np.bitwise_xor(kh,r)%(2**dim)
                        x=d[i,j,z]
                ...
                for i in range(d.shape[0]):
                        ...
                        kh=np.bitwise_xor(k,h)
                        d[i,:,z]=np.bitwise_xor(kh,r)%(2**dim)
                        x=d[i,:,z]
